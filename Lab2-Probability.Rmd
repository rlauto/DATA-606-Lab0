...
```{r 1, eval=TRUE}
# A streak length of 1 consists of 2 shot attempts and means a single hit followed by one miss. A streak length of zero is just one shot attempt that missed.
```
...
```{r 2, eval=TRUE}
# The distribution of Kobe's streak has a right skew with the most frequent streak length being 0 and his longest streak being 4.
```
## Compared to What?
...
```{r 4, eval=TRUE}
# The `prob` argument needs to be added to the sample function and set equal to `c(.45,.55)`.
...
```{r, eval=TRUE}
test_streak <- calc_streak(sim_basket)
barplot(table(test_streak), main = "Simulation Streak Distribution")
# Similar to Kobe, the distribution of streaks is right skewed and the most frequent streak length was 0. The longest streak of baskets is 4.
```
...
```{r, eval=TRUE}
# Rerunning the simulation of the independent shooter a second time would result in a similar streak distribution to the first simulation because the sample function is given the same arguments as the first time but there may be variance in sampling results due to chance.
sim_basket2 <- sample(outcomes, size = 133, replace = TRUE, prob = c(.45,.55))
test_streak2 <- calc_streak(sim_basket2)
barplot(table(test_streak2), main = 'Rerun of Simulation')
```
...
```{r, eval=TRUE}
# Kobe's distribution of streak lengths is follows a similar trend to the simulated independent shooter. As the streak length increases the frequency of the streak decreases. Comparing the simulation to Kobe's distribution does not support the hypothesis that Kobe's shooting pattern follows the hot hand model. If Kobe's shooting pattern followed the hot hand model we would expect to see a higher frequency of streaks with a length of two.
  
par(mfrow=c(1,2))
barplot(table(kobe_streak), main = "Kobe's Streak Distribution")
barplot(table(test_streak), main = "Simulation Streak Distribution")
```
