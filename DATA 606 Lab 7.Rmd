...
```{r 1, eval=F}
I would use a scatter plot. The relationship looks linear but I am unsure if there is a strong correlation and whether I am comfortable using a linear model to predict the number of runs.
```
```{r 1.1, eval = T}
...
```{r 2, eval = F}
As `at_bats` increase so do the number of `runs`. The positive correlation indicates an upward trend but not a bery strong one. There also is an outlier that seems unusual with the number of at bats around 5510 and over 850 recorded runs.
```
...
```{r 3, eval = F}
After running the function 3 times, 125360.5, was the smallest sum of squares I created.
```
...
```{r 5}
prediction <- -2789.2429+(0.6305*5578)
prediction

# 5579 is the closest number of at bats to 5578

# overestimate by residual:
residual <- mlb11$runs[mlb11$at_bats==5579] - prediction
residual
```
## Model diagnostics
...
```{r 6, eval = F}
The residules seem to have no upward or downward trend, staying near the horizontal line at 0, however they do seem to be fairly variable.
```
...
```{r 7, eval = F}
Yes, the distribution appears to be normal.
```
...
8.  Based on the plot in (1), does the constant variability condition appear to 
    be met?
```{r 8, eval = F}
Yes, because all of the residuals are scattered around 0.
```
* * *
...
# linear model
lm_bat_avg <- lm(runs ~ bat_avg, mlb11)
summary(lm_bat_avg)

abline(lm_bat_avg)

# Yes, there seems to be a linear relationship between bat_avg and runs
```
-   How does this relationship compare to the relationship between `runs` and 
    ...
    tell?
```{r, eval=F}
R^2 for at bats is 0.3729
R^2 for batting average is 0.6561
Batting average seems to predict runs better than at bats based on the higher R^2 value
```
-   Now that you can summarize the linear relationship between two variables, 
    ...
    five).
```{r}
# batting average best predicts runs because it has the highest R^2 value
...
# linear model
lm_new_onbase <- lm(runs ~ new_onbase, mlb11)
summary(lm_new_onbase)

abline(lm_new_onbase)
```
```{r}
...
```
```{r , eval=F}
On base percentage looks to be a big improvement when compared to the batting average.
```
```{r}
# scatter plot
...
# linear model
lm_new_slug <- lm(runs ~ new_slug, mlb11)
summary(lm_new_slug)
...
```
```{r , eval=F}
Slugging percentage looks to be even better than on base percentage
```
...
# linear model
lm_new_obs <- lm(runs ~ new_obs, mlb11)
summary(lm_new_obs)
...
```{R , eval = F}
Of all variables, the on base percentage plus the slugging percentage was the best predictor of runs, with an R^2 of 0.9349. Based on my knowledge of baseball this makes sense to me, getting on base is an important first step to scoring a run and getting doubles, triples, or homeruns of course help drive in runs.
```
