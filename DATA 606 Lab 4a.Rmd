...
```{r 1., eval=F}
The population distribution for area is slightly right skewed.
```
...
```
```{r, eval=F}
The sample of 50 also is slightly right skewed
```
...
```
```{r, eval=F}
`samp2` has a mean similar to `samp1`.
```
```{r, eval=T}
...
```
```{r eval=F}
As the sample size increases and approaches the population size the sample mean will provide a more accurate estimate of the population's mean.
```
...
```{r eval=F}
There are 5,000 elements in `sample_means50`, each one of them is a mean calculated using one of the 5,000 samples of size 50. If we increase the number of samples taken to 50,000 the distribution shape will become more normal.
```
```{r}
my_sample_means50 <- rep(NA, 50000)

for(i in 1:50000){
   samp <- sample(area, 50)
   my_sample_means50[i] <- mean(samp)
   }

hist(my_sample_means50, breaks = 25)
```
...
for (i in 1:100){
  ...
  sample_means_small[i] <- mean(small_samp)
}
...
```
```{r eval=F}
Each element represents a mean calculated for one of the 100 samples of 50 taken from the population.
```
...
```{r eval=F}
As the sample size gets larger the center of the distribution increases in frequency and the spread/standard deviation decreases.
```
...
```
```{r eval=F}
Based on teh distribution I would guess the mean to be around 180,000.
```
```{r}
mean(sample_means50)
mean(price)
...
```{r eval=F}
The sampling distribution is normal for `sample_means150` and has a smaller spread that `sample_means50` with a mean around 180,000
```
...
```{r eval=F}
`sample_means150 has a smaller spread because as the sample size becomes larger the standard deviation and spread of the distribution of means becomes smaller. Taking a larger sample size would ensure a smaller spread and estimates that are more often close to the true population values.
```
<div id="license">
