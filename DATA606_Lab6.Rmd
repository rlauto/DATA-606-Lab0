...
```{r eval=F}
The percentages appear to be sample statistics describing the results of the latest total PositionFill
```
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    assumption?
```{r eval = F}
It must be assumed that each observation is independent, participants were selected at random, and the sample size is large enough.
```
...
```{r eval=F}
Each row in table 6 corresponds to a country that was sampled for the poll while `atheism` corresponds to an individuals response to the poll.
```
...
```{r eval=F}
We must assume observation independence, random sampling, and the success failure condition has to be met.
```
```{r}
# np>=10
sum(athcount$freq) * ((athcount$freq[athcount$x == 'TRUE']) / sum(athcount$freq))
# n(1-p)>=10
sum(athcount$freq) * ((athcount$freq[athcount$x == 'FALSE']) / sum(athcount$freq))
```
...
## np>=10
sum(Auscount$freq) * ((Auscount$freq[Auscount$x == 'TRUE']) / sum(Auscount$freq))
## n(1-p)>=10
sum(Auscount$freq) * ((Auscount$freq[Auscount$x == 'FALSE']) / sum(Auscount$freq))
...
## np>=10
sum(cancount$freq) * ((cancount$freq[cancount$x == 'TRUE']) / sum(cancount$freq))
## n(1-p)>=10
sum(cancount$freq) * ((cancount$freq[cancount$x == 'FALSE']) / sum(cancount$freq))
...
```{r eval=F}
The max Margin of Error is at a proportion 0.5. As proportion approaches 0 and 1 the Margin of error decreases.
```
...
# Equador does not meet the conditions although n*p is 8 and not far below 10. However there are many other countries that could also have the same issue and makes me inclined to reject the results.
```
* * *
## On your own
...
H1 = Spain's atheism index has changed significantly from 2005 to 2012.
```
```{r}
...
```
```{r eval=F}
Confidence intervals overlap so I would accept the null hypothesis; Spain's atheism index has not changed.
```
...
H1 = United States' atheism index has changed significantly from 2005 to 2012.
```
```{r}
...
```
```{r eval=F}
Confidence intervals do not overlap so I would accept the alternate hypothesis; the United States' atheism index has changed.
```
...
```{r eval=F}
Simply multiply the significance level by 39, the number of countries in the table.
```
```{r}
0.05 * 39
```
